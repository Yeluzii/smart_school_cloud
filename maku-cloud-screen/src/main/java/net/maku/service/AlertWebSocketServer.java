...
public class AlertWebSocketServer {
    ...
    private static final Map<String, AlertWebSocketServer> CLIENTS = new ConcurrentHashMap<>();
...
//            }
     ...
     * 发送消息给指定客户端
     */
    public static void sendMessageToClient(String clientId, JSONObject message) {
        AlertWebSocketServer client = CLIENTS.get(clientId);
        if (client != null) {
            try {
                client.sendMessage(message);
            } catch (IOException e) {
    ...
    public static Map<String, AlertWebSocketServer> getClients() {
        return CLIENTS;
    }
}
